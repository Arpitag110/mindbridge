...
const JournalPage = () => {
    ...
    const handleSave = async () => {
        if (!content.trim()) return showToast("Journal content cannot be empty.", "error");
        setIsSaving(true);
    ...
    const handleDelete = async (id) => {
        const ok = await showConfirm("Delete this journal entry?");
        if (!ok) return;
        try {
                    ...
                    {entries.length === 0 ? (
                        ...
                        entries.map((entry) => (
                            <div
                                ...
                                className={`p-4 rounded-xl cursor-pointer transition-all border group relative ${selectedEntry && selectedEntry._id === entry._id
            ...
            <div className="flex-1 h-full flex flex-col relative">
                ...
                <div className="flex-1 max-w-4xl mx-auto w-full p-6 md:p-12 flex flex-col">
                                ...
                                <div className="flex bg-white/60 rounded-lg p-1 border border-indigo-100 shadow-sm">
                                    {visibilityOptions.map((option) => (
                                        <button
                                            ...
                                            className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all ${visibility === option
                    ...
                    <textarea
                        ...
                        className={`flex-1 w-full bg-transparent resize-none focus:outline-none text-lg leading-relaxed text-gray-700 placeholder-indigo-200 custom-scrollbar ${selectedEntry ? "cursor-text" : ""
                            }`}
                        disabled={selectedEntry}
                    ></textarea>
