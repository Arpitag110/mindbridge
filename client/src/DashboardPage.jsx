      ...
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        ...
        <div className="relative group">
          ...
          <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-xl border hidden group-hover:block z-20 overflow-hidden">
            <div className="p-3 border-b text-xs font-bold text-gray-500 uppercase bg-gray-50">Recent Activity</div>
            <div className="max-h-60 overflow-y-auto">
              {notifications.length === 0 ? (
                <div className="p-4 text-center text-gray-400 text-sm">No new activity yet</div>
              ) : (
                notifications.map((n, i) => (
                  <div key={i} className="p-3 border-b hover:bg-indigo-50 transition text-sm flex gap-2">
                   ...
                   {recentChats.length > 0 ? (
                      recentChats.map(u => (
                        <div 
                           key={u._id} 
                           onClick={() => startChat(u)} 
                           className="flex items-center gap-3 cursor-pointer hover:bg-indigo-50 p-2 rounded-xl transition border border-transparent hover:border-indigo-100"
                        >
                           <div className="relative">
                             <img src={`https://api.dicebear.com/7.x/avataaars/svg?seed=${u.username}`} className="w-10 h-10 rounded-full bg-gray-100" alt="avatar" />
                           </div>
                           <div className="flex-1">
                             <span className="font-medium text-gray-800 block">{u.username}</span>
